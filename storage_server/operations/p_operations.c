...
ErrorCode ExecuteRead(ReadRequest* Req,int clientfd) {
    ...
    if(readFromFile(Req->path,buffer,1024)){
       return FAILURE;
    }
    printf("read called for path:Req:%s\n",Req->path);
    if(socketSend(clientfd,&buffer,sizeof(buffer))){
        eprintf("Could not Send Read Data to Client");
        return FAILURE;
    }
    return SUCCESS;
}
